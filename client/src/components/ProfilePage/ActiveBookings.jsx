...
function ActiveBookings({ activeBookings, setUpDate }) {
  return (
    <>
      <h6 className="offset-sm-2 offset-md-3 offset-lg-0">Mina nuvarande bokningar</h6>
      {activeBookings === undefined ? (
        <p className="d-flex flex-column align-items-center flex-lg-row flex-lg-wrap gap-3 m-0">Du har inga bokningar för tillfället</p>
      ) : (
        <Row>
          <Container className="d-flex flex-column align-items-center flex-lg-row flex-lg-wrap gap-3 m-0">
            {activeBookings?.map((booking, i) => (
              <Col lg={12} className="booking-card d-flex flex-shrink-0 mb-2 align-items-center" key={booking.referenceNumber + i}>
                <Col xs={3} sm={3} lg={3}>
